...
def _write_recovery_bundle(
    ...
    *,
    ...
    for relative_path, payload in files.items():
        ...
        if top in dropbox_sync._REQUIRED_PARQUET_DATASETS:
            entry.update(
                ...
                pit_min_utc="2026-07-04T20:00:00Z",
                pit_max_utc="2026-07-04T20:00:00Z",
            )
        elif top == "_manifests":
    ...
    manifest = {
        "schema": "swinglab_recovery_bundle_v1",
        "status": "complete",
...
def test_publish_recovery_bundle_uploads_ready_last(tmp_path: Path, monkeypatch) -> None:
    ...
    generation_root = "/root/date=2026-07-04/generations/generation-test-001"
    ...
    assert [remote for remote, _ in uploads] == [
        *(f"{generation_root}/{path}" for path in sorted(local_files)),
        ...
        "/root/date=2026-07-04/_READY.json",
    ]
    ready = json.loads(uploads[-1][1])
    assert ready["schema"] == "swinglab_recovery_ready_v1"
    assert ready["status"] == "ready"
    assert ready["session_date"] == "2026-07-04"
...
def test_publish_recovery_bundle_refuses_tampered_data(tmp_path: Path, monkeypatch) -> None:
    relative_path = "recommendations/date=2026-07-04/data.parquet"
...
def test_publish_recovery_bundle_requires_complete_v1_roles(
    tmp_path: Path,
    monkeypatch,
) -> None:
    archive, manifest_path, _ = _write_recovery_bundle(tmp_path)
...
def test_publish_recovery_bundle_upload_failure_never_writes_ready(
    ...
    assert "/root/date=2026-07-04/_READY.json" not in attempted_remote_paths
